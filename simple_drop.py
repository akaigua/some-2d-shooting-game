...
class Platform:
    ...
    def draw(self, surface):
        ...
        pygame.draw.rect(surface,  # draw the platform in white
                         (255,255,255),
                         (0,self.y,Controller.WINDOW_WIDTH,Platform.THICKNESS)
                        )
...
class Avastar:
    def __init__(self,platforms,WINDOW_WIDTH, WINDOW_HEIGHT):
    ...
    def collision(self, avatar_x, avatar_y, avatar_width):
        ...
        '''
        assert avatar_x >= 0
        assert avatar_x < self.WINDOW_WIDTH
        assert avatar_y >= 0
        assert avatar_y < self.WINDOW_HEIGHT
...
def main():
    ...
    platforms = Platform(WINDOW_WIDTH, WINDOW_HEIGHT)
    redbox = Avastar(platforms, WINDOW_WIDTH, WINDOW_HEIGHT)
    events = Controller()
    ...
    while events.running:
        ...
        if events.left_down:
            redbox_x = max(0, redbox_x-events.redbox_vx)
        if events.right_down:
            redbox_x = min(redbox_x+events.redbox_vx, WINDOW_WIDTH-REDBOX_WIDTH)
